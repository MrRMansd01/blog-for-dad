...
async function fetchAndPopulateNavCategories() {
    ...
    try {
        ...
        categories.forEach(cat => {
            ...
            link.href = `/articals.html?category=${cat._id}`;
...
async function fetchAndPopulateSlider() {
    ...
    try {
        ...
        articles.slice(0, 5).forEach(article => {
            ...
            slide.innerHTML = `<a href="artical_page.html?id=${article._id}"><img src="${article.imageUrl || 'https://placehold.co/900x400'}" alt="${article.title}"><div class="article-slide-caption"><h3>${article.title}</h3></div></a>`;
...
async function fetchAndDisplayLatestArticles(limit, containerId, categoryId = null) {
    ...
    try {
        ...
        articles.forEach(article => {
            ...
            const articleCard = `<article class="article-card"><div class="card-image"><a href="artical_page.html?id=${article._id}"><img src="${article.imageUrl || 'https://placehold.co/600x400'}" alt="${article.title}"></a></div><div class="card-content">${categoryHtml}<h3 class="card-title"><a href="artical_page.html?id=${article._id}">${article.title}</a></h3><p class="card-excerpt">${article.excerpt}</p><a href="artical_page.html?id=${article._id}" class="card-link">ادامه مطلب &larr;</a></div></article>`;
...
async function fetchAndDisplaySingleArticle(articleId) {
    ...
    try {
        ...
        if (article.category && article.category.name) metaText += ` | دسته‌بندی: <a href="/articals.html?category=${article.category._id}">${article.category.name}</a>`;
...
async function fetchAndDisplaySidebarPosts() {
    ...
    try {
        ...
        articles.slice(0, 5).forEach(article => {
            ...
            listItem.innerHTML = `<a href="artical_page.html?id=${article._id}">${article.title}</a>`;
...
async function fetchAndDisplaySidebarCategories() {
    ...
    try {
        const response = await fetch(API_URL_CATEGORIES);
        const categories = await response.json();
        categoriesList.innerHTML = `<li><a href="/articals.html">همه مقالات</a></li>`;
        categories.forEach(cat => {
            const listItem = document.createElement('li');
            listItem.innerHTML = `<a href="/articals.html?category=${cat._id}">${cat.name}</a>`;
